...
export function WorkspaceJoinScreen({ userEmail, onJoinWorkspace, onCheckWorkspaceExists, onCreateWorkspace }) {
  ...
  const [workspaceExists, setWorkspaceExists] = useState(null);
  const [checkingWorkspace, setCheckingWorkspace] = useState(false);

  async function handleCheckWorkspace(event) {
    event.preventDefault();
    const normalizedName = workspaceName.trim().toLowerCase().replace(/[^a-z0-9-_]/g, '');

    if (!normalizedName || checkingWorkspace) {
      setError("Workspace name must contain only letters, numbers, hyphens, and underscores");
      return;
    }
    ...
    try {
      ...
      if (!result.exists) {
        // Workspace doesn't exist, proceed to create
  ...
  async function handleJoinWithSecret(event) {
    event.preventDefault();
    ...
    try {
      await onJoinWorkspace(normalizedName, secret.trim().toUpperCase(), userEmail);
    } catch (err) {
      const message = String(err?.message || "");
      if (message.includes("invalid_secret")) {
        setError("Invalid workspace secret");
      } else if (message.includes("workspace_not_found")) {
  ...
  async function handleCreateWorkspace() {
    ...
    try {
      await onCreateWorkspace(normalizedName, userEmail);
    } catch (err) {
        ...
        {workspaceExists === null ? (
          <form onSubmit={handleCheckWorkspace}>
            <div className="login-field">
              <label className="login-label" htmlFor="workspace-name">Workspace Name</label>
              <input
                id="workspace-name"
                ...
                disabled={checkingWorkspace || loading}
                placeholder="e.g. my-team"
                pattern="[a-z0-9-_]+"
                title="Only lowercase letters, numbers, hyphens, and underscores allowed"
              />
            </div>
            <button className="login-button" type="submit" disabled={checkingWorkspace || loading}>
              {checkingWorkspace ? "Checking..." : "Next"}
            </button>
            {error ? <p className="login-error">{error}</p> : null}
          </form>
        ) : (
          <form onSubmit={handleJoinWithSecret}>
            <div className="login-field">
              <label className="login-label" htmlFor="workspace-name-display">Workspace Name</label>
              <input
                id="workspace-name-display"
                className="login-input"
                type="text"
                value={workspaceName}
                disabled
            ...
            <div className="login-field">
              <label className="login-label" htmlFor="workspace-secret">Workspace Secret</label>
              <input
                id="workspace-secret"
                className="login-input"
                type="text"
